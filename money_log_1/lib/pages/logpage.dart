...
class _LogpageState extends State<Logpage> {
  ...
  void onSave(double amount  , String? category , String description){
    ...
    setState(() {
    var logbook = Provider.of<LogBook>(context , listen: false);  
    var log = Log(amount: amount , description: description , category: category?? "ERROR" );
    logbook.addLog(widget.name,log);
  ...
  void onDelete(Log log){
    setState(() {
    var logbook = Provider.of<LogBook>(context , listen: false);  
    logbook.removeLog(widget.name,log);
    });
  }
  ...
  void edit(Log log , double amount  , String? category , String description ){
      setState(() {
        var logbook =  Provider.of<LogBook>(context , listen: false);
        logbook.editLog( widget.name ,log, description, category, amount);
      });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      title: Text(widget.name.toUpperCase() , style: TextStyle(color: Colors.white , fontSize: 22),),
      ),
      body: Column(
